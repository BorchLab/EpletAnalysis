...
df <- process_hla_registry(file = "./data/HMM-CL01-0620-CAT-00.xlsx", 1)
sab <- process_sab_data(file = "./data/SAB_Class_I_Lot_14_20221212.xlsx", c(1,2))
pra <- process_pra_data(file = "./data/PRA_Class_I_Lot_020_20221212.xlsx", 1)
mix <- process_mix_data(file = "./data/LSM12NC23_024_01.xlsx", 1)
explex <- process_explex_data(file = "./data/ExPlex_classI_II.xlsx", 1)
...
generate_venn_diagram(df, "outputs/viz/Figure2_classI.png", pattern = "_unique")

# Filter antibody-verified data and generate another Venn diagram
dfa <- df %>% filter(`Antibody Reactivity` == "Confirmed")
generate_venn_diagram(dfa, "outputs/viz/Figure3_classI.png", pattern = "_unique")

# Save final output
write.xlsx(df, "outputs/files/ph_counts_eplets_class1.xlsx")
```
...
# Save final output
write.xlsx(df, "outputs/files/ph_counts_eplets_class2.xlsx")
```
...
#Making Data Frames
sab.frame <- calculate_sum(df, sab$Molecular.Typing, "SAB_sum", return.sum = FALSE)
sab.frame <- sab.frame%>% 
                  select(-c('Alleles','Antibody Reactivity','Class','Exposed','Sero Group','AA Position')) %>%
                  as.data.frame() %>%
...
explex.frame <- calculate_sum(df, explex$Molecular.Typing, "EXPLEX_sum", return.sum = F)
...
pra.frame <- calculate_sum(df, pra$values, "PRA_sum", return.sum = F)
pra.frame <- pra.frame%>% 
                  select(-c('Alleles','Antibody Reactivity','Class','Exposed','Sero Group','AA Position')) %>%
                  as.data.frame() %>%
                  replace_with_colnames_and_col4() %>%
...
mix.frame <- calculate_sum(df, mix$values, "MIX_sum", return.sum = F)
mix.frame <- mix.frame %>% 
                  select(-c('Alleles','Antibody Reactivity','Class','Exposed','Sero Group','AA Position')) %>%
                  as.data.frame() %>%
                  replace_with_colnames_and_col4() %>%
...
upset(
  upset_data,
  intersect = colnames(upset_data), # Now in alphabetical order
  base_annotations = list('Number of Combined\nEplet-Antigens' = intersection_size()),
  sort_intersections_by='degree',
  themes = upset_default_themes(text = element_text(size = 12)), 
  set_sizes = FALSE, 
  sort_sets = FALSE 
